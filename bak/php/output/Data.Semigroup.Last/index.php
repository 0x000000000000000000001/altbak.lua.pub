...
if (!function_exists(__NAMESPACE__ . '\\phpurs_eval_thunk')) {
  function phpurs_eval_thunk($id) {
    ...
    switch ($id) {
      case 'Data_Semigroup_Last_append': $v = (function() {
...
$show = $__case_res_0;
    $__res = (object)["show" => (function() use ($__global_Data_Semigroup_Last_append, $show) {
  $__body = function($v) use ($__global_Data_Semigroup_Last_append, $show) {
    $__case_0 = $v;
    if (true) {
$a = $__case_0;
return ($__global_Data_Semigroup_Last_append)("(Last ", ($__global_Data_Semigroup_Last_append)(($show)($a), ")"));
} else {
throw new \Exception("Pattern match failure");
};
  };
  $__fn = function($v) use ($__global_Data_Semigroup_Last_append, $show, $__body, &$__fn) {
  $__num = func_num_args();
  if ($__num < 1) {
    return phpurs_curry_fallback($__fn, func_get_args(), 1);
  }
